...
export async function scrapeProducts(page: Page): Promise<ScrapedProduct[]> {
  ...
  const products = await page.evaluate(`(() => {
    ...
    for (const card of amazonCards) {
      ...
      let price = '';
      if (wholeEl) {
        const whole = (wholeEl.textContent && wholeEl.textContent.replace(/[^0-9,]/g, '')) || '';
        const fraction = (fractionEl && fractionEl.textContent && fractionEl.textContent.replace(/[^0-9]/g, '')) || '00';
      ...
      if (!price) {
        ...
        price = (offscreen && offscreen.textContent && offscreen.textContent.trim()) || '';
      }
      ...
      const originalPrice = (origEl && origEl.textContent && origEl.textContent.trim()) || '';
        ...
        if (trimName && trimName.length >= 3 && trimPrice) {
          ...
          if (!seen.has(key)) {
            ...
            if (resolvedUrl && !resolvedUrl.startsWith('http')) {
              try { resolvedUrl = new URL(resolvedUrl, pageUrl).href; } catch (e) { resolvedUrl = ''; }
            }
            results.push({ name: trimName, price: trimPrice, url: resolvedUrl, onSale: onSale, originalPrice: originalPrice.trim() });
          }
        }
      }
    }
